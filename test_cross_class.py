...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str,
        default='output_dir/vit_base_16_checkpoint.pth', help='checkpoint path of model')
...
def main():
    ...
    UNSEEN_OBJECTS = ['bottle', 'cable', 'capsule', 'screw', 'transistor']
    args = parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    args.device = device
    
    args.model_path = 'output_dir/vit_base_16_obj_to_texture/checkpoint_976_931.pth'
    ...
    protoflow = ProtoFlow('weights/protoflow/weights_texture.pt')
    
    all_image_aucs, all_pixel_aucs, mask_ratios = [], [], []
    for class_name in UNSEEN_TEXTURES:
    ...
    for i, class_name in enumerate(UNSEEN_TEXTURES):
    ...
    # args.model_path = 'output_dir/vit_base_16_obj2/checkpoint-299.pth'
    # model = prepare_model(args, args.model_path)
    # print('Model loaded.')
    # model.to(device)
    
    # tokenizer = utils.create_d_vae(
    #     weight_path=args.tokenizer_weight_path, d_vae_type=args.discrete_vae_type,
    #     device=device, image_size=args.second_input_size)
    
    # protoflow = ProtoFlow('weights/protoflow/weights_obj2.pt')
    
    # all_image_aucs, all_pixel_aucs, mask_ratios = [], [], []
    # for class_name in UNSEEN_OBJECTS:
    #     args.class_name = class_name
    ...
    #     all_pixel_aucs.append(pixel_auc)
    #     mask_ratios.append(mask_ratio)
    # for i, class_name in enumerate(UNSEEN_OBJECTS):
    #     print(f'{class_name}: Image-AUC: {all_image_aucs[i]}, Pixel-AUC: {all_pixel_aucs[i]}, Mask-Ratio: {mask_ratios[i]}')
    # print('Mean Image-AUC: {}'.format(np.mean(all_image_aucs)))
    # print('Mean Pixel-AUC: {}'.format(np.mean(all_pixel_aucs)))
