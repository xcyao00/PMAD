...
class ProtoFlow(object):
    ...
    # def predict2(self, data_loader, class_name, mask_ratio=0.3):
    #     coreset = coreset_bank[class_name]
    #     index = faiss.IndexFlatL2(coreset.shape[1])
    #     index.add(coreset)
    #     res = faiss.StandardGpuResources()
    ...
    #     pre_scores = rank.new_zeros(rank.shape)
    #     pre_scores[pre_mask] = rank[pre_mask]
    #     pre_scores[~pre_mask] = 0  # unmask patch, no anomaly score
        
    #     pre_scores = pre_scores.reshape(-1, 14, 14)
    #     pre_scores = F.interpolate(pre_scores.unsqueeze(1),
    #         size=224, mode='bilinear', align_corners=True).squeeze().cpu().numpy()
        
    #     img_scores = pre_scores.reshape(pre_scores.shape[0], -1).max(axis=1)
    #     gt_list = np.array(gt_label_list)
    #     gt_mask = np.squeeze(np.asarray(gt_mask_list, dtype=np.bool), axis=1)

    #     img_auc = roc_auc_score(gt_list, img_scores)
    #     pix_auc = roc_auc_score(gt_mask.flatten(), pre_scores.flatten())
        
    #     pre_mask = pre_mask.cpu().numpy()

    #     return pre_mask, img_labels, abnomality, img_auc, pix_auc