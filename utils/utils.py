...
from dall_e.tokenizer import Dalle_VAE, DiscreteVAE
...
def save_model_gan(args, epoch, model, model_without_ddp, discriminator, optimizer, d_optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('checkpoint-%s.pth' % epoch_name)]
...
def patchify(imgs, patch_size=16, num_chans=3):
    ...
    patches = imgs.reshape(shape=(imgs.shape[0], num_chans, h, patch_size, w, patch_size))
    patches = torch.einsum('nchpwq->nhwpqc', patches)
    patches = patches.reshape(shape=(imgs.shape[0], h * w, patch_size**2 * num_chans))
